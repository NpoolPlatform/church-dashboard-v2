<template>
  <q-table
    ...
    :columns='columns'
    @row-click='(ev, row, index) => onRowClick(row as pledge.Pledge)'
  >
    <template #top-right>
...
import { utils, sdk, pledge, goodbase } from 'src/npoolstore'
...
const target = ref({} as pledge.Pledge)
...
const onRowClick = (row: pledge.Pledge) => {
  showing.value = true
  updating.value = true
  target.value = row
}
...
const onMenuHide = () => {
  ...
  target.value = {} as pledge.Pledge
}

const onSubmit = () => {
  submitting.value = true
  updating.value ? updatePledge() : createPledge()
}
...
const createPledge = () => {
  sdk.adminCreatePledge(target.value, (error:boolean) => {
    submitting.value = false
    if (error) return
    onMenuHide()
  })
...
const updatePledge = () => {
  sdk.adminUpdatePledge(target.value, (error:boolean) => {
    submitting.value = false
    if (error) return
    onMenuHide()
  })
...
const columns = computed(() => [
  ...
  },
  {
    ...
    sortable: true,
    field: (row: pledge.Pledge) => row.ContractDeploymentAddress
  },
  {
    name: 'ContractCalculateAddress',
    label: t('MSG_CONTRACT_CALCULATE_ADDRESS'),
    sortable: true,
    field: (row: pledge.Pledge) => row.ContractCalculateAddress
  },
  {
    name: 'BenefitType',
  ...
  },
  {
    ...
    sortable: true,
    field: (row: pledge.Pledge) => row.ContractCodeURL
  },
  {
    name: 'ContractCodeBranch',
    label: t('MSG_CONTRACT_CODE_BRANCH'),
    sortable: true,
    field: (row: pledge.Pledge) => row.ContractCodeBranch
  },
  {
    name: 'ContractState',
    label: t('MSG_CONTRACT_STATE'),
    sortable: true,
    field: (row: pledge.Pledge) => row.ContractState
  },
  {
    name: 'StartAt',
    label: t('MSG_STARTAT'),
  ...
  },
  {
    ...
    sortable: true,
    field: (row: pledge.Pledge) => utils.formatTime(row.CreatedAt)
  }
])
