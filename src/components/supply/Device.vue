<template>
  ...
  >
    <q-card class='popup-menu'>
      ...
      <q-card-section>
        <q-input v-model='target.ManufacturerID' :label='$t("MSG_MANUFACTURER")' />
...
const updateDevice = () => {
  sdk.adminUpdateDeviceType(target.value, (error: boolean) => {
    if (error) return
    onMenuHide()
  })
}
...
const createDevice = () => {
  sdk.adminCreateDeviceType(target.value, (error: boolean) => {
    if (error) return
    onMenuHide()
  })
}
